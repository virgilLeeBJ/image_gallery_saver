...
class ImageGallerySaverPlugin(private val registrar: Registrar) : MethodCallHandler {
    ...
    override fun onMethodCall(call: MethodCall, result: Result): Unit {
        when {
            ...
            call.method == "getAndroidAlbumPath" -> {
                result.success(getAndroidAlbumPath())
            }
    ...
    private fun generateFileByFileName(orgFile: File): File {
        val storePath = Environment.getExternalStorageDirectory().absolutePath + File.separator + getApplicationName()
        val appDir = File(storePath)
        ...
        Log.d("TESTTEST", "fileName--->" + fileName)
        return File(appDir, fileName)
    }
    ...
    private fun saveFileToGallery(filePath: String): String {
        ...
        return try {
            ...
            val file = generateFileByFileName(originalFile)
    ...
    private fun getAndroidAlbumPath(): String {
